    ...
    public partial class ExamQuestForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)//submit
        {
            ...
            else//submit
            {
                ...
                var chars = from c in ent.Choices select c;
                var newList = chars.Where(x => answers.Contains(x.Cho_Content)).Select(x => x.Cho_Char);
                
                string[] charachters= newList.ToArray();
                MessageBox.Show("you submitted your exam");
                this.Close();
                //exam correction
                ent.ExamAnswers(examID, studentID, charachters[0], charachters[1], charachters[2], charachters[3], charachters[4], charachters[5], charachters[6], charachters[7], charachters[8], charachters[9]);
